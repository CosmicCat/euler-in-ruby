...
$numPaths = 2 ** $tree.length-1

$pathSums = Array($numPaths)

def sumPath(binString)
  sum = $tree[0][0].to_i
  lastX = 0
  for i in 1..$tree.length-1
    lastX += binString[i].to_i
    sum += $tree[i][lastX].to_i
  end
  return sum
end

#each path through this tree can be represented by a unique binary number
def main
  for i in 0..$numPaths-1
    # convert to bin, pad with zeroes
    binString = i.to_s(2).rjust($tree.length,'0')
    $pathSums[i] = sumPath(binString)
  end
  puts $pathSums.max
end
